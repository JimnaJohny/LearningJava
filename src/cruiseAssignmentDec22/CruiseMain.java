...
public class CruiseMain {
	...
	public static void main(String[] args) {
		...
		userDetails.setPhoneNumber(phoneNumberByUser,sc);
		System.out.println("Enter your email address.");
		String emailIdByUser = sc.next();
		userDetails.setEmailId(emailIdByUser,sc);
		...
		userDetails.setPassWord(passWordByUser,sc);
		...
		userDetails.logIn(userNameToSignIn, passWordToSignIn,sc);
...
//UPDATED
		cruise.setNumberOfChildrenAboveFive(noOfChildren,sc);
		...
		if (changePersonalInfo.equalsIgnoreCase("Y")) {
			...
			userDetails.isPassWordValidated(passwordToBeValidated,sc);
			...
			if (true) {
				...
				do {
					...
					if (infoToBeChanged.equals("1")) {
						...
						userDetails.setPhoneNumber(newphoneNumber,sc);
